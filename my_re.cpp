...
NFA::~NFA()
{
	for(decltype(this->ValidStats.size()) i=0;i<this->ValidStats.size();i++)
...
void NFA::DeleteEpsilon()
{
	...
	for (decltype(this->ValidStats.size()) i = 0; i < this->ValidStats.size(); i++)
	{
		...
		for (decltype(OutEdgeList.size()) j = 0; j < OutEdgeList.size(); j++)
	...
	for (decltype(this->ValidStats.size()) i = 0; i < this->ValidStats.size(); i++)
		for (list<Edge>::iterator it = this->ValidStats[i]->OutEdges.begin(); it != this->ValidStats[i]->OutEdges.end();)
			if (it->MatchContent == epsilon)
				...
				it++;
	for (decltype(this->unValidStats.size()) i = 0; i < this->unValidStats.size(); i++)
...
RegularExpr::RegularExpr(char* pattern)
{
	dfa = new DFA(pattern);
}

char* RegularExpr::GetMathchedPstr(char* src)
{
	int len = dfa->scan(src);
	return src + len;
}

char* RegularExpr::GetMatchedStr(char* src)
{
	int len = dfa->scan(src);
